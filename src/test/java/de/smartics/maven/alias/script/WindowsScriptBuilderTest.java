...
public class WindowsScriptBuilderTest {
  ...
  public void checksEnvironmentsAndRejectsAliasIfNoMatch() {
    final Alias alias = new Alias.Builder().withName(ALIAS_NAME)
        .withCommand(ALIAS_COMMAND).withEnv("linux").build();
  ...
  public void checksEnvironmentsAndAcceptsAliasIfMatch() {
    final Alias alias = new Alias.Builder().withName(ALIAS_NAME)
        .withCommand(ALIAS_COMMAND).withEnv("windows").build();
    final AliasGroup group = createAliasGroup(alias);
    uut.addAliases(group);
    final String script = uut.createScript();
  ...
  public void checksForNotPassingArgs() {
    final Alias alias = new Alias.Builder().withName(ALIAS_NAME)
        .withCommand(ALIAS_COMMAND).withPassArgs(false).build();
    final AliasGroup group = createAliasGroup(alias);
    uut.addAliases(group);
    final String script = uut.createScript();
    assertTrue(script.contains("doskey any = command"));
    assertTrue(script.contains(
        "doskey h   = echo  --- test ^& echo  any = command ^& echo  --- help ^& echo  h   = This help."));
  }
  ...
  public void appliesExtensions() {
    ...
    final AliasExtension extension = (new AliasExtension.Builder())
        .withName("xx").withTemplate("xxx {@cmd} yyy")
        .addGroup(ALIAS_GROUP_NAME).withComment("Does xxx and yyy.").build();
    ...
    final String expectedCommand =
        "doskey anyxx = xxx " + ALIAS_COMMAND + " yyy";
    final String script = uut.createScript();
